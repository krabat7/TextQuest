...
<c:if test="${area == FOREST}">
    <h1>Лес</h1>
    <p>Вы вошли в густой лес, где деревья высоки и мрачны. Странные звуки донеслись из-за деревьев, и ветви создают густые тени на земле.</p>
...
<c:if test="${area == WATERFALL}">
    ...
    <p>Вдруг, за водопадом Вы видите проход. Но идти туда большая опасность. Не ясно, что скрывается в нем.</p>
</c:if>

<c:if test="${area == PORTAL}">
    <h1>Портал</h1>
    <p>Вы перед загадочным порталом. Вокруг портала ничего необычного, но на стене вы замечаете изображение ключа.</p>
</c:if>
